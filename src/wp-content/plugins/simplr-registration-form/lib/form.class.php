...
class SREG_Form {
	...
	static function checkbox2($option, $vals, $class) {
		?>
		<div class="option-field <?php echo apply_filters($option['name'].'_error_class',''); ?>">
		<?php $sel = ('on' == $vals)?'checked':''; ?>
		<label class="checkbox" for="<?php echo $option['name']; ?>">	<input type="checkbox" name="<?php echo $option['name']; ?>" id="<?php echo $option['name']; ?>" value="on" class="<?php echo @$class; ?>" <?php echo $sel; ?>/>
			<?php echo $option['label'] . show_required($option); ?>
		</label>
	...
	// total hack para habilitar múltiplos checks
	static function checkbox($option,$vals,$class,$options_array) {
		?>
		<?php $vals = (!is_array($vals)) ? array($vals) : $vals; ?>
			...
			<div class="checkbox-wrapper">
				...
				<?php foreach($options_array as $k => $v): ?>
					...
					<label class="checkbox">
						<input type="checkbox" name="<?php echo $option['name']; ?>[]" value="<?php echo esc_attr($k); ?>" <?php echo (in_array($k,$vals))? 'checked': ''; ?>> <?php echo $v; ?></input>
					</label>
					</div>
				<?php endforeach; ?>
			</div>
	...
	static function radio($option, $vals, $class,$options_array) {
		...
		<label for="<?php echo $option['name']; ?>"><?php echo $option['label'] . show_required($option); ?></label>
			...
			<?php if(is_numeric($k)) { $k = $v; } ?>
				...
				<label class="radio">
				<input type="radio" name="<?php echo $option['name']; ?>" value="<?php echo esc_attr($k); ?>" <?php echo $sel; ?>/>
				<?php echo $v; ?> 
				</label>
			<?php endforeach; ?>
		</div>
	...
	static function date($option, $vals, $class, $options_array) { 
		...
		<label for="<?php echo $option['name']; ?>"><?php echo $option['label'] . show_required($option); ?></label>
		<div class="simplr-clr"></div>
		<select class="span1" style="width:60px;"  name="<?php echo $option['name'] . '-dy'; ?>" id="<?php echo $option['name']. '-dy'; ?>" class="<?php echo @$class; ?>">
		...
		<select class="span2" name="<?php echo $option['name']. '-mo'; ?>" id="<?php echo $option['name']. '-mo'; ?>" class="<?php echo @$class; ?>">

		<option value="">Mês</option>
		<option value="01" <?php if($lmo == '01') {echo 'selected'; } ?>>Janeiro</option>
		<option value="02" <?php if($lmo == '02') {echo 'selected'; } ?>>Fevereiro</option>
		<option value="03" <?php if($lmo == '03') {echo 'selected'; } ?>>Março</option>
		<option value="04" <?php if($lmo == '04') {echo 'selected'; } ?>>Abril</option>
		<option value="05" <?php if($lmo == '05') {echo 'selected'; } ?>>Maio</option>
		<option value="06" <?php if($lmo == '06') {echo 'selected'; } ?>>Junho</option>
		<option value="07" <?php if($lmo == '07') {echo 'selected'; } ?>>Julho</option>
		<option value="08" <?php if($lmo == '08') {echo 'selected'; } ?>>Agosto</option>
		<option value="09" <?php if($lmo == '09') {echo 'selected'; } ?>>Setembro</option>
		<option value="10" <?php if($lmo == '10') {echo 'selected'; } ?>>Outubro</option>
		<option value="11" <?php if($lmo == '11') {echo 'selected'; } ?>>Novembro</option>
		<option value="12" <?php if($lmo == '12') {echo 'selected'; } ?>>Dezembro</option>
		</select>
	
		<select class="span1" style="width:70px;" name="<?php echo $option['name'] . '-yr'; ?>" id="<?php echo $option['name']. '-yr'; ?>" class="<?php echo @$class; ?>">
