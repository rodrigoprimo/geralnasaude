...
function simplr_validate($data,$atts) {
	...
	if('yes' == @$atts['password']) 
	{
		...
		//	$errors[] = __('You must enter a password to register for this site.', 'simplr-reg');
			$errors[] = __('<div class="alert alert-alert">Você deve digitar uma senha para se registrar.</div>', 'simplr-reg');
			add_filter('password_error_class','_sreg_return_error');
		}
	}
...
//	$errors[] = __("You must enter a username.",'simplr-reg'); 
	$errors[] = __("<div class='alert alert-alert'>Digite um nome de usuário.</div>",'simplr-reg'); 
	add_filter('username_error_class','_sreg_return_error');
		...
		$user_test = validate_username($data['username']);
			if($user_test != true) {
					$errors[] .= __('<div class="alert alert-alert">Nome de usuário inválido.</div>','simplr-reg');
					add_filter('username_error_class','_sreg_return_error');
				}
		...
		foreach($fields as $cf) 
		{
			...
			if($field['required'] == 'yes') 
			{
				...
				{ 
					...
					{
					$errors[] = '<div class="alert alert-error"><i>&ldquo;'. $field['label'] .'&rdquo;</i> &eacute; um campo obrigat&oacute;rio. Por favor preencha-o.</div>';
...
function simplr_send_notifications($atts, $data, $passw) {
	...
	if( @$simplr_options->mod_on == 'yes' AND @$simplr_options->mod_activation == 'auto')  {
		...
		wp_mail($data['email'],"$name - Confirmação de registro", apply_filters('simplr_email_confirmation_message',$emessage), $headers);
	}
...
function simplr_build_form($data,$atts) {
	...
	$label_email = apply_filters('simplr_label_email', __('Email','simplr-reg') );
	$label_confirm_email = apply_filters('simplr_label_confirm_email', __('Confirme o Email','simplr-reg') );
	$label_username = apply_filters('simplr_label_username', __('Nome de usuário','simplr-reg') );
	$label_pass = apply_filters('simplr_label_password', __('Escolha uma senha','simpr-reg'));
	$label_confirm = apply_filters('simplr_label_confirm', __('Confirme sua senha','simpr-reg'));
	...
	$form .=  apply_filters('simplr-reg-submit', '<input type="submit" name="submit-reg" value="Enviar" class="submit button">');
