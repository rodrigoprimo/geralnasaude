...
//    require_once DW_TEMPLATE_PATH . 'inc/simple-avatar.php';
	...
	register_nav_menus( array(
		...
		'secondary' => __( 'Menu Secund&aacute;rio', 'dw_focus' ),
	) );
...
if( ! function_exists('dw_human_time_diff') ) {
    function dw_human_time_diff( $from, $format ){
        ...
        if($diff <= 1){
            ...
            $since = sprintf(_n('%s segundo', 'h&aacute; %s segundos', $diff), $diff);
        } else if ($diff <= 3600) {
        $mins = round($diff / 60);
        ...
        $since = sprintf(_n('h&aacute; %s minutos', 'h&aacute; %s minutos', $mins), $mins);
        } else if ( ($diff <= 86400) && ($diff > 3600)) {
            $hours = round($diff / 3600);
        if ($hours <= 1) {
            $hours = 1;
        }
        $since = sprintf(_n('h&aacute; %s horas', 'h&aacute; %s horas', $hours), $hours);
        } elseif ($diff >= 86400 && $diff <= 86400*2 ) {
            $days = round($diff / 86400);
        if ($days <= 1) {
            $days = 1;
        }
        $since = sprintf(_n('h&aacute; %s dia', 'h&aacute; %s dias', $days), $days);
        } else {
...
/* add_role('correspondente_educador', 'Correspondente educador', array(
    'read' => true, // True allows that capability
    'edit_posts' => true,
    'delete_posts' => true // Use false to explicitly deny
));

add_role('correspondente_doutor', 'Correspondente Doutores da Alegria', array(
    'read' => true, // True allows that capability
    'edit_posts' => true,
    'delete_posts' => true // Use false to explicitly deny
));

add_role('correspondente_interfarma', 'Correspondente Interfarma', array(
    'read' => true, // True allows that capability
    'edit_posts' => true,
    'delete_posts' => true // Use false to explicitly deny
));
...
add_role('parceiros', 'Parceiros', array(
    'read' => true, // True allows that capability
    'edit_posts' => true,
    'delete_posts' => true // Use false to explicitly deny
));

add_role('correspondente_outro', 'Correspondente', array(
    'read' => true, // True allows that capability
    'edit_posts' => true,
    'delete_posts' => true // Use false to explicitly deny
)); */
...
remove_role('correspondente_jovem');
remove_role('correspondente_educador');
remove_role('correspondente_doutor');
remove_role('correspondente_interfarma');
remove_role('parceiros');
remove_role('correspondente_outro');
remove_role('subscriber');
remove_role('colaborador_interfarma');
remove_role('colaborador_jovem');
remove_role('colaborador_interfarma');
remove_role('colaborador_doutor');
remove_role('colaborador_educador');
remove_role('author');
...
$(\'form#your-profile > table:eq(1)\').hide();
$(\'#post-by-email\').hide();
$(\'.option-field\').hide();
$(\'form#your-profile\').show();
$(\'label[for=url], input#url\').hide();
$(\'label[for=aim], input#aim\').hide();
$(\'label[for=yim], input#yim\').hide();
$(\'label[for=jabber], input#jabber\').hide();
$(\'form#your-profile .simplr-clr\').hide();
$(\'form#your-profile h3:eq(0)\').hide();
$(\'form#your-profile h3:eq(5)\').hide();
...
$(\'form#your-profile > table:eq(0)\').remove();
$(\'#post-by-email\').remove();
$(\'.option-field\').remove();
$(\'form#your-profile h3:eq(0)\').remove();
$(\'form#your-profile h3:eq(3)\').remove();
$(\'label[for=url], input#url\').remove();
$(\'label[for=aim], input#aim\').remove();
$(\'label[for=yim], input#yim\').remove();
$(\'label[for=jabber], input#jabber\').remove();
$(\'form#your-profile .simplr-clr\').remove();
...
function custom_login_logo() {
	...
	h1 a { background-image: url('.get_bloginfo('template_directory').'/assets/img/logo.png) !important;background-size: 230px 100px !important;height:100px !important; }
	</style>';
}
