	...
	function end_el( &$output, $item, $depth = 0, $args = array() ) {
		...
		for ($i=0; $i<count($item->children);$i++) {
			...
			$output .="<ul id='mn-latest-".$child->ID."'>";
				if($child->object == 'category'){
			        ...
			        $r = new WP_Query( apply_filters( 'widget_posts_args', 
			          ...
			          if ($r->have_posts()) :
							...
							$output.= "><div class='subcat-thumbnail'><a href='".get_permalink()."' title='".get_the_title()."'>".get_the_post_thumbnail( get_the_ID(), array(40,40))."</a></div><div class='subcat-title'><a href='".get_permalink()."' title='".get_the_title()."'> ".get_the_title()."</a><span> - ". human_time_diff( get_the_time('U'), current_time('timestamp') ) ." atr&aacute;s</span></div></li>";
					            } 
			  ...
			  $output .= "</ul>";
			  $output .= "<a href='".$child->url."' title='".$child->attr_title."'>Ver tudo</a>";
			  $output .= "</div>";
			}
